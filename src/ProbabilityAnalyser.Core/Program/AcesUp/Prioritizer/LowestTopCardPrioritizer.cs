...
{
	[DisplayName("by piles with lowest top card")]
	public class LowestTopCardPrioritizer : IPilePrioritizer
	{
		public virtual IEnumerable<AcesUpPile> Prioritize(AcesUpRunContext context)
		{
			...
			var pairs = topCards.OrderBy(x => x.Value.Rank, comparer).ToList();
