...
{
	[DisplayName("by pile without hidden ace(s) as left to right")]
	public class HasNoHiddenAcesPilePrioritizer : IPilePrioritizer
	{
		public IEnumerable<AcesUpPile> Prioritize(AcesUpRunContext context)
		{
			...
			var pairs = sizes.OrderBy(x => x.Value).ThenBy(x => x.Key).ToList();
